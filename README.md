...
| Plugin Path                  | `pluginPath`                | A list of plugins to be loaded on the server alongside the ones defined in the `plugins` directory. Contains the project JAR by default. | `${project.build.finalName}.jar` | Path is relative to the output directory of the project, or `target` if not specified. `null` can be used to disable the feature.           |
